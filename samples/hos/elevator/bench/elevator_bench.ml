...
module Car : sig
  ...
  (* Bench: sleeps stripped. State machine, locks, threads, and
     validated transitions are unchanged. *)
  let door_open d = d.is_open <- true
  let door_close d = d.is_open <- false
...
module Dispatcher : sig
  type t
  val make : num_cars:int -> ?on_state_change:(int -> car_state -> unit) -> unit -> t
  ...
  let make ~num_cars ?(on_state_change=fun _ _ -> ()) () =
    let extra_callback = on_state_change in
    ...
    let on_state_change car_id new_state =
      ...
      extra_callback car_id new_state
    in
...
(* ---------- Bench harness ---------- *)
(* Throughput bench: drive N alternating-floor requests through
   the unmodified-shape Dispatcher + Cars and measure wall time.
   Convention from the Python and Rust benches: 16 events per
   cycle (7 Car-observed + 9 Door/Motor/Dispatcher/BuildingSystem).

   Completion is counted via an atomic-style ref incremented from
   the on_state_change callback each time any car re-enters
   Idle_at_floor. When completions reach N, the timed run is done. *)

let () =
  let n = match Array.length Sys.argv with
    | a when a >= 2 -> int_of_string Sys.argv.(1)
    | _ -> 1000
  in
  let completions = ref 0 in
  let completions_mutex = Mutex.create () in
  let completions_cv = Condition.create () in
  let on_state_change _car_id new_state =
    match new_state with
    | Idle_at_floor ->
      Mutex.lock completions_mutex;
      incr completions;
      Condition.broadcast completions_cv;
      Mutex.unlock completions_mutex
    | _ -> ()
  in
  let dispatcher = Dispatcher.make ~num_cars:2 ~on_state_change () in
  ...
  for i = 0 to 9 do
    let floor = if i mod 2 = 0 then 5 else 1 in
    Dispatcher.request dispatcher floor Up
  done;
  Mutex.lock completions_mutex;
  while !completions < 10 do
    Condition.wait completions_cv completions_mutex
  done;
  completions := 0;
  Mutex.unlock completions_mutex;
  ...
  Printf.printf
    "OCaml correct: cars=2 N=%d wall=%.3fs events/sec=%.2f completions=%d\n"
    n elapsed rate !completions;
  Printf.printf
    "[BENCH] OCaml correct events/sec=%.2f cycles=%d wall=%.3fs\n"
    rate n elapsed;
